...
def mock_transcriber():
    ...
    mock_transcriber.transcribe.return_value = {
        "text": long_text,
        "segments": [{"start": 0.0, "end": 1.0, "text": "Something very long indeed."}]
    }
    return transcriber
...
def service(mock_config, mock_transcriber, fake_history):
    with patch("src.core.transcription_service.history_mgr", fake_history):
...
def test_transcribe_file_sync_logic(service, mock_transcriber, tmp_path):
    ...
    with patch("src.core.transcription_service.LLMFactory.create_client", return_value=fake_llm), \
