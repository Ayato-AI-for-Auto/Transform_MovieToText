...
def system_setup(tmp_path):
    ...
    with patch("src.controllers.transcription_ctrl.history_mgr", history), \
         patch("src.core.transcription_service.history_mgr", history):
...
def test_system_live_recording_flow(system_setup):
    ...
    # Mock recorders
    with patch("src.recorder.visual_recorder.visual_recorder.start"), \
         patch("src.recorder.visual_recorder.visual_recorder.stop"), \
         patch("src.core.live_processor.LiveTranscriptionManager.start"), \
         patch("src.core.live_processor.LiveTranscriptionManager.stop", return_value=("This is a very long transcription result that definitely exceeds both the fifty character limit for titles and the one hundred character limit for AI category extraction. It is long enough to trigger all AI logic paths. EXTRA TEXT FOR 100+.", [{"text": "Something long"}])):
        ...
        with patch("src.core.transcription_service.LLMFactory.create_client", return_value=mock_llm):
            # 1. Start
            ctrl.start_live_recording("base-model", "System")
            ...
            assert state.get("transcript_text") == "This is a very long transcription result that definitely exceeds both the fifty character limit for titles and the one hundred character limit for AI category extraction. It is long enough to trigger all AI logic paths. EXTRA TEXT FOR 100+."
            assert state.get("category") == "Live"
            
            # 4. Verify DB persistence
            meeting = history.get_meeting(m_id)
            assert meeting is not None
            assert meeting["transcript"] == "This is a very long transcription result that definitely exceeds both the fifty character limit for titles and the one hundred character limit for AI category extraction. It is long enough to trigger all AI logic paths. EXTRA TEXT FOR 100+."
