...
# Import target
from src.platforms.desktop.ui.ui_utils import _model_cache, sync_llm_models
...
def test_sync_llm_models_initial_fetch():
    ...
    config_mgr.get_llm_models.return_value = ["model1", "model2"]
...
def test_sync_llm_models_cache_expiry():
    ...
    config_mgr.get_llm_models.return_value = ["new_model"]
...
def test_sync_llm_models_fallback_on_error():
    ...
    with patch.object(config_mgr, "get_llm_models", side_effect=Exception("API Down")), patch("threading.Thread") as mock_thread:
        sync_llm_models(page, config_mgr, provider, dd_model)
        target_fn = mock_thread.call_args[1]["target"]
        target_fn()
